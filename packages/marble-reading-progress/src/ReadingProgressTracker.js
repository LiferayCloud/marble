...
class ReadingProgressTracker extends Scrollspy {
  ...
  handleActiveIndexChanged(data) {
    if (core.isDef(data.prevVal) && data.prevVal >= 0) {
      let prevElement = this.getElementForIndex(data.prevVal);
  ...
  handleProgressChanged(data) {
    let element = this.getElementForIndex(this.activeIndex);
  ...
  updateProgress() {
    ...
    if (index >= 0) {
      ...
      let maxBottom = this.getScrollHeight_() + this.offset;
      let bottom = Math.min(maxBottom, region.bottom);
